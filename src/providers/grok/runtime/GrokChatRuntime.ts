...
export class GrokChatRuntime implements ChatRuntime {
  ...
  async *query(
    ...
    if (!this.sessionId) {
      ...
      if (!sessionId) {
        yield { type: 'error', content: 'Failed to create a Grok session.' };
        yield { type: 'done' };
        return;
      }
    }
  ...
  private buildLaunchSpec(params: {
    ...
    const rootArgs = ['--no-auto-update'];
    if (params.permissionMode === 'plan') {
      rootArgs.push('--permission-mode', 'plan');
    }

    const agentArgs = ['agent'];
    if (params.model) {
      agentArgs.push('--model', params.model);
    }
    if (params.effort) {
      agentArgs.push('--reasoning-effort', params.effort);
    }
  ...
  private async startProcess(launchSpec: GrokLaunchSpec): Promise<void> {
  ...
  private async authenticate(initResponse: AcpInitializeResponse): Promise<void> {
    ...
    const method = methods.find(entry => entry.id === 'cached_token')
      ?? methods.find(entry => entry.id === 'xai.api_key');
    if (!method) {
      throw new Error('Grok is not authenticated. Run `grok login` in a terminal, then try again.');
    }
  ...
  private async syncInitializeMetadata(response: AcpInitializeResponse): Promise<void> {
    ...
    const commands = normalizeGrokCommands(meta.availableCommands);
    ...
    const nextModels = models.length > 0
      ...
      : currentRawModelId
        ...
        : [];
    if (!sameGrokModels(currentSettings.discoveredModels, nextModels)) {
      updateGrokProviderSettings(settingsBag, { discoveredModels: nextModels });
  ...
  private async syncSessionModelState(params: {
    ...
    const models = normalizeGrokDiscoveredModels(acpState.availableModels.map((model) => ({
    ...
    const settingsBag = this.plugin.settings as unknown as Record<string, unknown>;
    const currentSettings = getGrokProviderSettings(settingsBag);
    if (sameGrokModels(currentSettings.discoveredModels, models)) {
      return;
    }

    updateGrokProviderSettings(settingsBag, { discoveredModels: models });
    await this.plugin.saveSettings();
    this.refreshModelSelectors();
  }
  ...
  private async handlePermissionRequest(
    ...
    const decision = await this.approvalCallback(
      ...
      input,
      blockedPath
        ? 'Grok wants to access this path.'
        : `Grok wants permission to use ${formatPermissionLabel(toolName)}.`,
      {
  ...
  private resolveSelectedRawModelId(providerSettings: Record<string, unknown>): string | null {
    ...
    if (!isGrokModelSelectionId(selectedModel)) {
      return GROK_DEFAULT_MODEL_ID;
    }

    return decodeGrokModelId(selectedModel) ?? GROK_DEFAULT_MODEL_ID;
  }

  private resolveSelectedEffort(providerSettings: Record<string, unknown>): string {
    return normalizeGrokReasoningLevel(providerSettings.effortLevel);
  }
  ...
  private getActiveDisplayModel(): string | undefined {
    ...
    return rawId ? encodeGrokModelId(rawId) : undefined;
  }
...
function normalizeGrokCommands(value: unknown): SlashCommand[] {
...
function sameGrokModels(left: Array<{ contextWindow?: number; description?: string; label: string; rawId: string }>, right: Array<{ contextWindow?: number; description?: string; label: string; rawId: string }>): boolean {
