...
import type { DroidProviderState } from '../types';
import { getDroidState } from '../types';
import { buildDroidPromptBlocks, buildDroidPromptText } from './buildDroidPrompt';
...
export class DroidChatRuntime implements ChatRuntime {
  readonly providerId = DROID_PROVIDER_ID;
  ...
  getCapabilities(): Readonly<ProviderCapabilities> {
    return DROID_PROVIDER_CAPABILITIES;
  }
  ...
  prepareTurn(request: ChatTurnRequest): PreparedChatTurn {
    return {
      ...
      persistedContent: '',
      prompt: buildDroidPromptText(request, [], this.getDroidSystemPrompt()),
      request,
    };
  }
  ...
  async ensureReady(options?: ChatRuntimeEnsureReadyOptions): Promise<boolean> {
    const settings = getDroidProviderSettings(this.plugin.settings);
    if (!settings.enabled) {
      this.setReady(false);
      return false;
    }
    ...
    const resolvedCliPath = this.plugin.getResolvedProviderCliPath(DROID_PROVIDER_ID) ?? 'droid';
  ...
  async *query(
    ...
    if (!(await this.ensureReady())) {
      yield { type: 'error', content: 'Failed to start Droid. Check the CLI path and login state.' };
      yield { type: 'done' };
      return;
    }

    if (!this.connection) {
      yield { type: 'error', content: 'Droid runtime is not ready.' };
      yield { type: 'done' };
      return;
    }
    ...
    if (!this.sessionId) {
      ...
      if (!sessionId) {
        yield { type: 'error', content: 'Failed to create a Droid session.' };
        yield { type: 'done' };
        return;
      }
    }
    ...
    const promptPromise = this.connection.prompt({
      prompt: buildDroidPromptBlocks(
        turn.request,
        shouldBootstrapHistory ? previousMessages : [],
        this.getDroidSystemPrompt(),
      ),
      sessionId,
    }).then((response) => {
  ...
  buildSessionUpdates(params: {
    ...
    const existingState = params.conversation
      ? getDroidState(params.conversation.providerState)
      : null;
  ...
  private buildLaunchSpec(params: {
    ...
    if (shouldPassDroidModelToCli(params.model)) {
      args.push('-m', params.model!);
    }
    if (params.effort !== DROID_DEFAULT_REASONING_LEVEL) {
      args.push('-r', params.effort);
    }
    if (params.permissionMode === 'yolo') {
      args.push('--auto', 'medium');
    } else if (params.permissionMode === 'plan') {
      args.push('--use-spec', '--auto', 'low');
    }
    args.push('--output-format', 'acp');
  ...
  private async startProcess(launchSpec: DroidLaunchSpec): Promise<void> {
  ...
  private async authenticate(initResponse: AcpInitializeResponse): Promise<void> {
    ...
    const method = methods.find(entry => entry.type !== 'env_var' && entry.id === 'droid_login')
      ...
      ?? methods.find(entry => entry.type !== 'env_var' && entry.id.toLowerCase().includes('droid'))
      ?? methods.find(entry => entry.type !== 'env_var')
      ?? null;
    if (!method) {
      throw new Error('Droid is not authenticated. Run `droid` in a terminal and complete the browser login, then try again.');
    }
  ...
  private async syncInitializeMetadata(response: AcpInitializeResponse): Promise<void> {
    ...
    const commands = normalizeDroidCommands(meta.availableCommands);
    if (commands.length > 0) {
      this.setSupportedCommands(commands);
    }

    const modelState = getRecord(meta.modelState);
    const models = normalizeDroidDiscoveredModels(
    ...
    const currentSettings = getDroidProviderSettings(settingsBag);
    const nextModels = models.length > 0
      ? models
      : currentRawModelId
        ? [{ label: currentRawModelId, rawId: currentRawModelId }]
        : [];
    if (!sameDroidModels(currentSettings.discoveredModels, nextModels)) {
      updateDroidProviderSettings(settingsBag, { discoveredModels: nextModels });
  ...
  private async syncSessionModelState(params: {
    ...
    const models = normalizeDroidDiscoveredModels(acpState.availableModels.map((model) => ({
    ...
    const settingsBag = this.plugin.settings as unknown as Record<string, unknown>;
    const currentSettings = getDroidProviderSettings(settingsBag);
    if (sameDroidModels(currentSettings.discoveredModels, models)) {
      return;
    }

    updateDroidProviderSettings(settingsBag, { discoveredModels: models });
    await this.plugin.saveSettings();
    this.refreshModelSelectors();
  }
  ...
  private async applySelectedSessionConfig(params: {
    ...
    const targetThoughtLevel = this.resolveSelectedEffort(providerSettings);
    const thoughtLevelState = extractAcpSessionThoughtLevelState({ configOptions });
    if (
      thoughtLevelState.configId
      && thoughtLevelState.currentLevel !== targetThoughtLevel
      && thoughtLevelState.availableLevels.some(level => level.id === targetThoughtLevel)
    ) {
      const response = await this.connection.setConfigOption({
        configId: thoughtLevelState.configId,
        sessionId: params.sessionId,
        type: 'select',
        value: targetThoughtLevel,
      });
      configOptions = response.configOptions ?? configOptions;
    }
  ...
  private async handlePermissionRequest(
    ...
    const decision = await this.approvalCallback(
      ...
      input,
      blockedPath
        ? 'Droid wants to access this path.'
        : `Droid wants permission to use ${formatPermissionLabel(toolName)}.`,
      {
  ...
  private resolveSelectedRawModelId(providerSettings: Record<string, unknown>): string | null {
    const selectedModel = typeof providerSettings.model === 'string'
      ...
      : '';
    if (!isDroidModelSelectionId(selectedModel)) {
      return DROID_DEFAULT_MODEL_ID;
    }

    const rawModelId = decodeDroidModelId(selectedModel) ?? DROID_DEFAULT_MODEL_ID;
    return resolveDroidCliModelId(rawModelId);
  }

  private resolveSelectedEffort(providerSettings: Record<string, unknown>): string {
    return normalizeDroidReasoningLevel(providerSettings.effortLevel);
  }
  ...
  private resolveSelectedSessionMode(
    ...
    const candidates = this.resolvePermissionMode(providerSettings) === 'plan'
  ...
  private getActiveDisplayModel(): string | undefined {
    ...
    return rawId ? encodeDroidModelId(rawId) : undefined;
  }

  private getDroidSystemPrompt(): string {
    return typeof this.plugin.settings.systemPrompt === 'string'
      ? this.plugin.settings.systemPrompt
      : '';
  }

  private getDroidSystemPromptKey(): string {
    return computeSystemPromptKey({
      customPrompt: this.getDroidSystemPrompt(),
  ...
  private formatRuntimeError(error: unknown): string {
    const baseMessage = error instanceof Error ? error.message : 'Droid request failed';
    ...
    return `${message}\n\nDroid reported an HTTP/2 keepalive timeout. Check Droid login, VPN/proxy/network settings, then retry. If you use a proxy, add HTTP_PROXY/HTTPS_PROXY in the Droid provider environment.`;
  }
...
function normalizeDroidCommands(value: unknown): SlashCommand[] {
...
function sameDroidModels(left: Array<{ contextWindow?: number; description?: string; label: string; rawId: string }>, right: Array<{ contextWindow?: number; description?: string; label: string; rawId: string }>): boolean {
